...
var var11 = functions.database.ref('/notifications/reject').onWrite(event => {
  if (!event.data.val()) {
              console.log("Not a new write event");
              return;
          }
  // Get the list of device notification tokens.
  const getDeviceTokensPromise = admin.database().ref(`/notifications/reject/token`).once('value');
  const postId = admin.database().ref(`/notifications/reject/post`).once('value');
    ...
    return admin.messaging().sendToDevice(token, payload).then(response => {
      ...
      const payload = {
        notification: {
          ...
          body: `Votre chauffeur a rejeté votre demande de réservation. Cliquez ici pour vérifier.`,
          sound: 'default',
