...
var var11 = functions.database.ref('/notifications/cancelBook').onWrite(event => {
  if (!event.data.val()) {
              console.log("Not a new write event");
              return;
          }
  // Get the list of device notification tokens.
  const getDeviceTokensPromise = admin.database().ref(`/notifications/cancelBook/token`).once('value');
  const postId = admin.database().ref(`/notifications/cancelBook/post`).once('value');
    ...
    return admin.messaging().sendToDevice(token, payload).then(response => {
      ...
      const payload = {
        notification: {
          ...
          body: `Un passager a annulé sa réservation avec vous. Cliquez ici pour vérifier.`,
          sound: 'default',
